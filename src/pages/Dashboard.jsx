...
export default function Dashboard() {
  ...
  const loadLeads = async () => {
    setLoading(true);
    try {
      const data = await Lead.filter({}, { sort: "-created_date", limit: 200 });
      setLeads(Array.isArray(data) ? data : []);
    } catch (e) {
      console.error("Error cargando leads:", e);
      try {
        const data2 = await Lead.list();
        setLeads(Array.isArray(data2) ? data2 : []);
      } catch (e2) {
        showToast("Error cargando leads: " + e2.message, "error");
      }
    }
    setLoading(false);
  };
  ...
  const handleSearch = async () => {
    if (!searchForm.ubicacion.trim()) {
      showToast("Escribe una ubicación primero", "error");
      return;
    }
    setSearching(true);
    ...
    try {
      ...
      setSearchResult(res.summary || res);
      await loadLeads();
      showToast(`✅ Búsqueda completada! ${res.summary?.leads_guardados || 0} leads nuevos.`);
      setTab("leads");
    } catch (e) {
  ...
  const filteredLeads = leads.filter(l => {
    ...
    if (filter.search) {
      ...
      const haystack = [l.nombre_negocio, l.categoria, l.ubicacion_buscada].join(" ").toLowerCase();
    ...
    <div style={{ minHeight: "100vh", background: "#F0F4F8", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      {toast && (
        <div style={{
          ...
          fontSize: "14px", fontWeight: 600, maxWidth: 380
        }}>{toast.msg}</div>
      )}

      {/* Header */}
      <div style={{ background: "linear-gradient(135deg, #0F2540 0%, #1A5276 100%)", color: "white", padding: "0 24px", boxShadow: "0 2px 16px rgba(0,0,0,0.2)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 0" }}>
          <div>
            <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.5px" }}>🌐 Flixih Lead System</div>
            <div style={{ fontSize: 12, opacity: 0.65, marginTop: 2 }}>Roy Lorenzo · Websites para PR</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={handleGeneratePreviews} disabled={generating} style={{
              background: generating ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.15)",
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", gap: 2 }}>
          {[
            ...
            <button key={t.key} onClick={() => setTab(t.key)} style={{
              ...
              color: tab === t.key ? "#0F2540" : "rgba(255,255,255,0.7)",
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 20px" }}>
        ...
        {tab === "stats" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16 }}>
            {[
              { label: "Total Leads", value: stats.total, color: "#0F2540", icon: "📋" },
              ...
              { label: "Enviados", value: stats.enviados, color: "#7C3AED", icon: "✅" },
            ].map(s => (
              <div key={s.label} style={{
                ...
                <div style={{ fontSize: 34, fontWeight: 800, color: s.color, margin: "8px 0 4px" }}>{s.value}</div>
                <div style={{ fontSize: 11, color: "#6B7280", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px" }}>{s.label}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {tab === "buscar" && (
          <div style={{ background: "white", borderRadius: 16, padding: 28, boxShadow: "0 2px 10px rgba(0,0,0,0.07)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16, marginBottom: 24 }}>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>📍 Ubicación</label>
                <input
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>🔢 Máx. Leads</label>
                <input
                  type="number"
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>⭐ Mín. Reviews</label>
                <input
                  type="number"
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>🏷️ Categorías</label>
                <input
                  value={searchForm.categorias}
                  onChange={e => setSearchForm({ ...searchForm, categorias: e.target.value })}
                  placeholder="restaurant,beauty_salon,dentist..."
                  style={{ width: "100%", padding: "11px 14px", border: "1.5px solid #E5E7EB", borderRadius: 10, fontSize: 14 }}
                />
              </div>
            </div>
            ...
            <button
              ...
              style={{
                ...
                boxShadow: searching ? "none" : "0 4px 14px rgba(15,37,64,0.3)"
              }}
            >{searching ? "⏳ Buscando negocios en Google Maps..." : "🚀 Iniciar Búsqueda"}</button>
              ...
              <div style={{ marginTop: 24, background: "#F0FDF4", border: "1.5px solid #BBF7D0", borderRadius: 14, padding: 22 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12 }}>
                  {[
                    ...
                    <div key={k} style={{ background: "white", borderRadius: 10, padding: "14px 10px", textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          ...
          <div>
            ...
            <div style={{
              ...
              <select value={filter.priority} onChange={e => setFilter({ ...filter, priority: e.target.value })}
                style={{ padding: "8px 12px", border: "1.5px solid #E5E7EB", borderRadius: 8, fontSize: 13, color: "#374151" }}>
              ...
              <span style={{ fontSize: 13, color: "#9CA3AF", fontWeight: 600 }}>{filteredLeads.length} lead{filteredLeads.length !== 1 ? "s" : ""}</span>
            </div>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: 80, color: "#94A3B8", background: "white", borderRadius: 16, boxShadow: "0 1px 6px rgba(0,0,0,0.06)" }}>
                <div style={{ fontSize: 52, marginBottom: 16 }}>🔍</div>
                <div style={{ fontWeight: 800, fontSize: 18, color: "#374151" }}>No hay leads todavía</div>
                <div style={{ marginTop: 8, fontSize: 14 }}>Ve a la pestaña "Buscar" y encuentra negocios en PR</div>
                <button onClick={() => setTab("buscar")} style={{
                  marginTop: 20, background: "#0F2540", color: "white",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {filteredLeads.map(lead => {
                  ...
                  const borderColor = PRIORITY_COLORS[lead.lead_priority]?.dot || "#CBD5E1";
                  return (
                    <div key={lead.id} style={{
                      background: "white", borderRadius: 14, padding: "16px 20px",
                      boxShadow: isOpen ? "0 4px 20px rgba(0,0,0,0.1)" : "0 1px 6px rgba(0,0,0,0.06)",
                      borderLeft: `4px solid ${borderColor}`,
                      cursor: "pointer", transition: "box-shadow 0.2s"
                    }} onClick={() => setSelectedLead(isOpen ? null : lead)}>
                      ...
                      {/* Expanded Detail */}
                      {isOpen && (
                        <div style={{ marginTop: 18, borderTop: "1.5px solid #F3F4F6", paddingTop: 18 }} onClick={e => e.stopPropagation()}>
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(190px, 1fr))", gap: 16, marginBottom: 16 }}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: 13 }}>
                                {lead.website_url
                                  ? <a href={lead.website_url} target="_blank" rel="noopener" style={{ color: "#2980B9", wordBreak: "break-all" }}>{lead.website_url.substring(0, 45)}{lead.website_url.length > 45 ? "..." : ""}</a>
                                  : <span style={{ color: "#9CA3AF" }}>Sin website</span>}
                              </div>
                            </div>
                            <div>
                          ...
                          {lead.email_asunto && (
                            <div style={{ background: "#F0F9FF", border: "1.5px solid #BAE6FD", borderRadius: 12, padding: 16, marginBottom: 14 }}>
                              <div style={{ fontSize: 11, fontWeight: 800, color: "#0369A1", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 8 }}>📧 Email de Outreach</div>
                          ...
                          {lead.preview_html && (
                            <div style={{ marginBottom: 14 }}>
                              <button onClick={() => {
                                ...
                                if (w) { w.document.write(lead.preview_html); w.document.close(); }
                              }} style={{
                                background: "linear-gradient(135deg, #0F2540, #1A5276)",
                                color: "white", border: "none",
                          ...
                          <div style={{ marginBottom: 12 }}>
                            ...
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                              {Object.entries(OUTREACH_COLORS).map(([s, c]) => (
                                <button key={s} onClick={() => handleUpdateLead(lead.id, { outreach_status: s })} style={{
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              style={{
                                ...
                                fontSize: 13, minHeight: 64, resize: "vertical",
                                fontFamily: "inherit"
                              }}
                            />
                          </div>
